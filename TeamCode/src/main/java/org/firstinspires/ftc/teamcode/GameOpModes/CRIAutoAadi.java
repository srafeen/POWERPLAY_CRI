...
@Autonomous(name = "CRI Auto", group = "00-Autonomous" , preselectTeleOp = "TeleOp")
public class CRIAutoAadi extends LinearOpMode {
    ...
    //Middle
    TrajectorySequence trajOffWallMiddletoShipping;
    TrajectorySequence[] trajOffWallMiddleToBarCode = new TrajectorySequence[3];
    ...
    public void buildAutoStorage(){
        if (GameField.playingAlliance == GameField.PLAYING_ALLIANCE.BLUE_ALLIANCE) {
            ...
            carousalToAlliancePathPose1 = new Pose2d(-42,-13, Math.toRadians(180));
            //alShippingHubPose = new Pose2d(-38, -21 , Math.toRadians(-145));

            alShippingHubPose = new Pose2d(-17, -21, Math.toRadians(170));  //THIS IS Cooperative Hub postion
            ...
            carousalToAlliancePathPose1 = new Pose2d(46,-9,Math.toRadians(10)); //x=1, y=-60
            //alShippingHubPose = new Pose2d(33.5, -23.5, Math.toRadians(-45));
            alShippingHubPose = new Pose2d(17, -21, Math.toRadians(10));
            storageAlShippingHubDropDuckPose = new Pose2d(18, -31, Math.toRadians(-90));
    ...
    public void buildAutoMiddle(){
        if (GameField.playingAlliance == GameField.PLAYING_ALLIANCE.BLUE_ALLIANCE) {
            //BLUE_STORAGE_STARTPOS =  Pose2d(-61,-40,Math.toRadians(180));
            initPose = GameField.BLUE_MIDDLE_STARTPOS;
            offWallPose = new Pose2d(-56,-30.5,Math.toRadians(180));
            barcodePose[1-1] = new Pose2d(-48, -30, Math.toRadians(-170));
            barcodePose[2-1] = new Pose2d(-50, -38, Math.toRadians(180));
            barcodePose[3-1] = new Pose2d(-50, -48, Math.toRadians(165));
            ...
            alShippingHubPose = new Pose2d(-32, -26, Math.toRadians(-130)); //-34 for x
            storageAlShippingHubDropDuckPose = new Pose2d(-23, -33, Math.toRadians(-90));

            middleToStorageMidPose = new Pose2d(-22, -48, Math.toRadians(90));
            storageParkingPose = new Pose2d(-22,-124, Math.toRadians(90)); //x=-38,70

            whAlongWallParkingPose[0] = new Pose2d(-63, -32, Math.toRadians(90)); //x -69
            whAlongWallParkingPose[1] = new Pose2d(-72, 10, Math.toRadians(90)); //x -70
            whAlongWallParkingPose[2] = new Pose2d(-72, 45, Math.toRadians(90)); //x=-70
            ...
            whThroughBarrierParkingPose[0] = new Pose2d(-45, -20, Math.toRadians(95));
            whThroughBarrierParkingPose[1] = new Pose2d(-43, 60, Math.toRadians(90));
            whThroughBarrierShShippingParkingPose = new Pose2d(-40, 72, Math.toRadians(180));

        } else {
            //RED_STORAGE_STARTPOS =  Pose2d(61,-40,Math.toRadians(0));
            initPose = GameField.RED_MIDDLE_STARTPOS;
            offWallPose = new Pose2d(56,-29,Math.toRadians(0));
            barcodePose[1-1] = new Pose2d(49,-39, Math.toRadians(-5)); //fixed 1/8/22
            barcodePose[2-1] = new Pose2d(49,-39, Math.toRadians(-35)); //fixed 1/8/22
            barcodePose[3-1] = new Pose2d(48, -34, Math.toRadians(-50)); //fixed 1/8/22
            ...
            //alShippingHubPose = new Pose2d(33.5, -23.5, Math.toRadians(-45));
            alShippingHubPose = new Pose2d(34, -25.5, Math.toRadians(-45));
            storageAlShippingHubDropDuckPose = new Pose2d(18, -31, Math.toRadians(-90));
            middleToStorageMidPose = new Pose2d(15, -48, Math.toRadians(100));
            storageParkingPose = new Pose2d(15, -124, Math.toRadians(90)); //x = 26;
            ...
            whThroughBarrierParkingPose[0] = new Pose2d(53, -25, Math.toRadians(100)); //x=50
            whThroughBarrierParkingPose[1] = new Pose2d(43, 60, Math.toRadians(90));
            whThroughBarrierShShippingParkingPose = new Pose2d(40, 80, Math.toRadians(0));
        }
       ...
       /*trajInitToOffWall = driveTrain.trajectorySequenceBuilder(initPose)
        ...
        //Move from Carousal to Alliance Shipping Hub
        trajOffWallMiddletoShipping = driveTrain.trajectorySequenceBuilder(offWallPose/*driveTrain.getPoseEstimate()*/)
                .addTemporalMarker(0, () -> {
                    magazine.moveMagazineToTransport();
                    moveElevatorToTargetZoneLevel();
                })
                //.lineToLinearHeading(carousalToAlliancePathPose1) //Avoid Capstone
                .setVelConstraint(getVelocityConstraint(35, DriveConstants.MAX_VEL, DriveConstants.TRACK_WIDTH))
                .UNSTABLE_addTemporalMarkerOffset(1.0,() -> {
                    magazine.moveMagazineToDrop();
                })
                .lineToLinearHeading(alShippingHubPose)
        ...
        if (parkingLocation == GameField.PARKING_LOCATION.STORAGE) {
            trajASAlShippingToStorageParking = driveTrain.trajectorySequenceBuilder(alShippingHubPose)
                    .lineToLinearHeading(middleToStorageMidPose) //Avoid Capstone
                    .addTemporalMarker(1, () -> {
                        moveElevatorToLevel(0);
                    })
                    .lineToLinearHeading(storageParkingPose)
                    .build();
        } else if (parkingLocation == GameField.PARKING_LOCATION.NO_PARKING) {
            trajASAlShippingToStorageParking = driveTrain.trajectorySequenceBuilder(alShippingHubPose)
                    .lineToLinearHeading(whThroughBarrierParkingPose[0]) //Avoid Capstone
                    .addTemporalMarker(1, () -> {
                        moveElevatorToLevel(0);
                    })
                    .build();
    ...
    public void runAutoMiddle(){
        ...
        //Move To Shipping Unit
        driveTrain.followTrajectorySequence(trajOffWallMiddletoShipping);


        //Move to Parking
        if (parkingLocation == GameField.PARKING_LOCATION.STORAGE) {
            while (gameTimer.time() < 24000) { //Wait till end of Autonomous mode
                safeWait(100);
            }
            driveTrain.followTrajectorySequence(trajASAlShippingToStorageParking);
        } else if (parkingLocation == GameField.PARKING_LOCATION.NO_PARKING) {
    ...
    public boolean senseIntakeCollectAndStop(){
        return magazine.getMagazineColorSensorState() == Magazine.MAGAZINE_COLOR_SENSOR_STATE.LOADED;
    }
    ...
    public void selectGamePlan(){
        ...
        while (!isStopRequested()) {
            ...
            if (gamepadController.gp1GetButtonXPress()) {
                GameField.startPosition = GameField.START_POSITION.MIDDLE;
                telemetry.addData("StartPose : ", GameField.startPosition);
                break;
            }
        ...
        if (GameField.playingAlliance == GameField.PLAYING_ALLIANCE.RED_ALLIANCE) {
            switch(GameField.startPosition){
                case STORAGE:
                    ...
                    break;
                case MIDDLE:
                    startPose = GameField.RED_MIDDLE_STARTPOS;
                    break;
                case WAREHOUSE:
            ...
            switch(GameField.startPosition){
                case STORAGE:
                    ...
                    break;
                case MIDDLE:
                    startPose = GameField.BLUE_MIDDLE_STARTPOS;
                    break;
                case WAREHOUSE:
        ...
        if (GameField.startPosition == GameField.START_POSITION.MIDDLE) {
            while (!isStopRequested()) {
                ...
                telemetry.addData("  WAREHOUSE PARK ALONG WALL (X)","");
                telemetry.addData("  WAREHOUSE PARK THROUGH BARRIER (B)","");
                telemetry.addData("  NO PARKING (Y)","");
                ...
                if (gamepadController.gp1GetButtonYPress()) {
                    autonomousRoute = GameField.AUTONOMOUS_ROUTE.NOT_APPLICABLE;
                    parkingLocation = GameField.PARKING_LOCATION.NO_PARKING;
                    telemetry.addData("Autonomous Route : ", autonomousRoute);
                    telemetry.addData("Parking Location: ",parkingLocation);
                    break;
                }
